...
class C3NotGate(Gate):
    """C3NotGate instruction."""

    def __init__(self, ctl1, ctl2, ctl3, tgt, circ=None):
        """Create new c3x instruction."""
        super().__init__("c3x", [], [ctl1, ctl2, ctl3, tgt], circ)
    ...
    def reapply(self, circ):
        ...
        self._modifiers(circ.c3x(self.qargs[0], self.qargs[1], self.qargs[2], self.qargs[3]))


def c3x(self, ctl1, ctl2, ctl3, tgt):
    """Apply c3x to circuit.
    If qargs is None, applies to all the qbits.
    Args is a list of QuantumRegister or single qubits.
    For QuantumRegister, applies c3x to all the qbits in that register."""
    return self._attach(C3NotGate(ctl1, ctl2, ctl3, tgt, self))


QuantumCircuit.c3x = c3x